...
class Opener extends NbAccessory {
  constructor (bridge, params) {
    params.category = bridge.Accessory.Categories.VIDEO_DOORBELL
    params.model = 'Opener'
    super(bridge, params)
  }

  update (state) {
    this.log('%s%s', state.stateName, state.ringactionState ? ', ring' : '')
  }

  async identify () {
    const response = this.client.lockState(this.id, NbClient.DeviceTypes.OPENER)
    this.update(response.body)
  }
}
