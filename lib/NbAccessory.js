...
class Bridge extends homebridgeLib.AccessoryDelegate {
  constructor (platform, context) {
    ...
    this.client.on('response', (id, statusCode, statusMessage, body) => {
    ...
    this.client.on('error', (error, id, method, resource, body, url) => {
      this.warn('request %d: %s %s', id, method, resource)
      this.warn('request %d: error: %s', id, error)
    })
  ...
  async checkSubscription () {
    if (this.callbackId == null) {
      ...
      const response = await this.client.callbackAdd(this.callbackUrl)
      if (response == null) {
        return
      }
    ...
    if (this.callbackId != null) {
      this.error('lost subscription to event notifications')
...
class SmartLock extends NbAccessory {
  constructor (bridge, params) {
    ...
    this.sensorService = new NbService.Sensor(this)
    this.historyService = new homebridgeLib.ServiceDelegate.History.Contact(
      this, { name: this.name },
      this.sensorService.characteristicDelegate('contact'),
      this.sensorService.characteristicDelegate('timesOpened'),
      this.sensorService.characteristicDelegate('lastActivation')
    )
  }
  ...
  update (state) {
    ...
    this.sensorService.update(state)
