  ...
  constructor (log, configJson, homebridge) {
    ...
    try {
      ...
      this.discovery
        .on('error', (error) => {
          this.warn(
            '%s: request %d: %s %s', this.discovery.name, error.request.id,
            error.request.method, error.request.resource
          )
          this.warn(
            '%s: request %d: error: %s', this.discovery.name, error.request.id, error
          )
        })
        .on('request', (request) => {
          this.debug(
            '%s: request %d: %s %s', this.discovery.name, request.id,
        ...
        .on('response', (response) => {
          this.vdebug(
            '%s: request %d: response: %j', this.discovery.name, response.request.id,
            response.body
          )
          this.debug(
            '%s: request %d: %d %s', this.discovery.name, response.request.id,
